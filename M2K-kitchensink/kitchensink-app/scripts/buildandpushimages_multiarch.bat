...
:DEFAULT_REGISTRY
    ...
    SET REGISTRY_NAMESPACE=kitchensink-app
	GOTO MAIN
...
echo "building and pushing image kitchensink"
pushd source\kitchensink\kitchensink
docker buildx build --platform ${PLATFORMS} -f Dockerfile --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/kitchensink .
popd
